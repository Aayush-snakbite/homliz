...
export default function CommercialPage() {
  ...
  const commercialProperties = useMemo(() => {
    return tenantDataService.getPublicProperties().filter((p) => {
      ...
      // SubCategory filter
      if (subCategory !== 'all') {
        if (subCategory === 'Shop' && p.subType !== 'Commercial Shop') return false;
        if (subCategory === 'Office' && p.subType !== 'Office Space' && p.subType !== 'Commercial Floor') return false;
        if (subCategory === 'Showroom' && p.subType !== 'Showroom') return false;
        if (subCategory === 'Warehouse' && p.subType !== 'Warehouse') return false;
        if (subCategory === 'Building' && p.subType !== 'Commercial Building') return false;
      }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 border-b border-white/10 pb-6">
            <div className="flex items-center space-x-2 text-indigo-400 font-semibold text-xs uppercase tracking-wider mb-2">
              ...
              <span>Commercial & Business Collection</span>
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight mb-2">
              Commercial Spaces for Rent in Gorakhpur
            </h1>
            <p className="text-sm text-slate-300">
              Prime retail shops, office spaces, high-footfall showrooms, warehouses, and commercial buildings in Golghar, Medical Road & Mohaddipur.
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-[#0F1626] border border-white/10 p-1.5 rounded-2xl">
              {[
                ...
                { id: 'Building', label: 'Commercial Buildings' },
              ].map((tab) => (
                <button
                  key={tab.id}
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center space-x-2 bg-[#0F1626] border border-white/10 px-3 py-2 rounded-xl text-xs">
                ...
                >
                  ...
                  <option value="Mohaddipur" className="bg-[#0F1626]">Mohaddipur</option>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  placeholder="Search shop, office..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="bg-[#0F1626] border border-white/10 rounded-xl pl-8 pr-3 py-2 text-xs text-white placeholder-slate-500 focus:border-indigo-500 focus:outline-none"
                />
              </div>
            </div>
          </div>
          ...
          {commercialProperties.length > 0 ? (
            ...
            <div className="text-center py-20 px-6 bg-[#0F1626] border border-white/10 rounded-3xl max-w-md mx-auto">
              ...
              <h3 className="text-xl font-bold text-white mb-2">No Commercial Spaces Found</h3>
              <p className="text-xs text-slate-400 mb-6">
                Try selecting &ldquo;All Commercial&rdquo; or clearing locality filters to view available business rentals in Gorakhpur.
              </p>
              <button
                onClick={() => {
                  setSubCategory('all');
